...
- **Three card backs, picked by shape and then by `game`** (`cardBackUrl`): a square collection carries the square back whichever game it belongs to, and everything else is 5:7 and picks between `pistols`' own back and the guest one. Shape comes first because a back is a *picture* — it has to be rasterized at the aspect of the mesh it goes on or it is visibly stretched, which is also why **a shape with no back file of its own gets a stretched one**: a fourth `aspect` in `contracts.json` needs a fourth back in `card-art.ts`. All three are pinned once by `CardTable`, not per deck — a back arriving late would flash blank stock across a whole table.
- **A collection's shape is `contracts.json`'s `aspect`** (`CHAIN.md` §2), carried to the felt as `TableDeck.aspect` and read together with a file deck's `DeckArt.aspect` through `CardTable`'s `deckAspect()`. It cuts the deck's cards, its empty slot and its back, so all three agree. Absent means the token card; `1` is the only other shape shipped. A shape only ever cuts a card **down** — it is fitted inside the default card's box, `ENGINE.md` §4 — so the grid's column pitch and framed height hold whatever a collection declares.
