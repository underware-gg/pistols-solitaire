...
Binding for all code under `client/src/engine/` and for any page that draws cards. Read [`CODING_STYLE.md`](./CODING_STYLE.md) first — this document adds to it, never overrides it. For the two tables built on top, see [`SOLITAIRE.md`](./SOLITAIRE.md) (the rules layer and `/solitaire`) and [`DECKS.md`](./DECKS.md) (`/decks` and `/deck/<slug>`).
...
- **Distances are in card heights.** `CARD_HEIGHT` is 1 world unit, so a gap of `1.1` is a tenth of a card of felt. `CARD_HEIGHT` and `CARD_WIDTH` are the *default* card's — a layout for a deck of another shape measures with `cardWidth(aspect)`/`cardHeight(aspect)`, see §4.
...
- **One geometry per aspect ratio**, memoized: `cardGeometry(aspect)`. Several decks of different shapes are in play — Torii's 5:7 token art (`TOKEN_ASPECT`), the 2:3 painted deck in `public/deck/` (`STANDARD_ASPECT`), and whatever a collection declares as `contracts.json`'s `aspect` (`SQUARE_ASPECT`, so far) — and stretching one onto another's mesh is obvious at a glance.
- **Whatever its shape, a card is cut to fit inside the default card's box** — never wider than `CARD_WIDTH`, never taller than `CARD_HEIGHT`, and as big as it can be within both. A card at least as wide as the default keeps the full width and is cut shorter (square, `0.8`); a narrower one keeps the full height and is cut narrower (`0.65`, the 2:3 deck). Both bounds are load-bearing: every table pitches its columns by `CARD_WIDTH` and frames its rows by `CARD_HEIGHT`, so a card that broke either would overlap its neighbours or stand higher than the card back the table is framed for. It is also what a real deck looks like — one deal, one card back, whatever is printed on the fronts.
...
- **Dimensions go through `cardWidth(aspect)` / `cardHeight(aspect)`.** A layout for a non-default deck must never use `CARD_WIDTH` or `CARD_HEIGHT`, which are the *default* card's — `solitaire-layout.ts` derives `WIDTH` and `HEIGHT` at the top for exactly this and uses neither constant anywhere. Same for `cardCornerRadius(aspect)`, which follows the width so a narrow card is not more rounded than a wide one.
...
- **It is the shape of the card that goes in it**, `aspect` and all — a slot still cut to the default shape under a square deck is the tell that a shape has been threaded to the cards and not to the felt. Same rule as the mesh: one width, the height follows.
...
```ts
<Deck3D label sublabel notice action cards cardPose back aspect pose hoverPose visible onSelect />
```

- `label`/`sublabel` hang in **front** of the deck — `sublabel` is any node, not just a string, because "how many cards" is sometimes not yet a number (`/decks` puts a `Spinner` there while Torii is still counting); `notice` is any DOM node drawn **over** it, centred on the deck's own anchor and taking no pointer events — a mark the table wants on a deck (`/decks` puts the free starter pack's on the empty Duelists deck — `DECKS.md` §6) whose meaning the engine never learns. All of them go when `visible` does.
  - **`action` is the one that is a control**, hung under the caption at `ACTION_DROP`: a button about *this deck*, so it travels with the deck and leaves with it instead of floating over the middle of the felt (`/decks` claims and buys packs there — `DECKS.md` §6). Its layer is `pointer-events-none` and each control turns its own back on, the same contract the page chrome keeps.
  - **It hangs downward from its anchor rather than being centred on it** — `w-max` and a 1px height, because `center`'s `translate3d(-50%,-50%,0)` resolves against the element's *own* box: half of a max-content width centres it on the deck, half of one pixel leaves its top edge on the anchor. So `ACTION_DROP` is the gap above the control and not a guess at its middle, which is what makes it checkable against the felt each layout leaves in front of its nearest deck (measured: ~26px to spare at the tightest window and deck count, with the caption cleared by ~22px).
