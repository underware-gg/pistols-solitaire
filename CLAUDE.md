...
- **`pnpm dev:claude` (port 3009), never `pnpm dev` (3000)** — 3000 is the user's own server. Stop only what you started (`lsof -ti:3009 | xargs kill`); never `pkill next dev`. `dev:claude` sets `NEXT_DIST_DIR=.next-claude` because Next locks one dev server per build dir; it appends `.next-claude/**` to `client/tsconfig.json` include — leave that.
